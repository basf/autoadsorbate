...
def run_generate(
    ...
    try:
        ...
        pop_trj = s.get_populated_sites(fragment, site_index='all', sample_rotation=True, mode='heuristic', conformers_per_site_cap=5, overlap_thr=1.6, verbose=True)
        for atoms in pop_trj:
            atoms.info['uid'] = str(uuid.uuid4())
    
        write(f"./generated_{atoms.info['adsorbate_info']['adsorbate_formula']}_{smiles}.xyz", pop_trj)
        print(f"at file {len(glob('./generated*xyz'))} / {len(files)}. Generated {len(pop_trj)} structures for smiles {smiles}")
...
Parallel(n_jobs=64)(delayed(run_generate)(file, slab, touch_sphere_size, to_init_dict) for file in files)
